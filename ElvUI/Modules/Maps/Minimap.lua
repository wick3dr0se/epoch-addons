...
function M:Initialize()
	...
	-- Brighten pfQuest route dots by intercepting Minimap:CreateTexture so any
	-- OVERLAY texture that pfQuest later marks with the route image gets 2× RGB.
	do
		local pfqRouteTextures = {}
		local origCreateTexture = Minimap.CreateTexture
		Minimap.CreateTexture = function(self, name, layer, ...)
			local tex = origCreateTexture(self, name, layer, ...)
			if layer == "OVERLAY" then
				local origST  = tex.SetTexture
				local origSVC = tex.SetVertexColor
				tex.SetTexture = function(t, path, ...)
					if type(path) == "string" and path:find("img\\route", 1, true) then
						pfqRouteTextures[t] = true
					end
					return origST(t, path, ...)
				end
				tex.SetVertexColor = function(t, r, g, b, a)
					if pfqRouteTextures[t] then
						r = math.min(r * 2, 1)
						g = math.min(g * 2, 1)
						b = math.min(b * 2, 1)
					end
					return origSVC(t, r, g, b, a)
				end
			end
			return tex
		end
